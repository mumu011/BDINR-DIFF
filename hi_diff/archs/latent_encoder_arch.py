...
class latent_encoder_gelu(nn.Module):
    ...
    def forward(self, inp_img, gt=None):
        if gt is not None:
            x = torch.cat([gt, inp_img], dim=1)  # [B, 6, H, W]
        else:
            x = inp_img

        x = self.pixel_unshuffle(x) # [B, 6*16, H//4, W//4]
        x = self.conv1(x)  # [B, 64, H//4, W//4]
        for block in self.blocks:
            x = block(x) + x  # [B, 64, H//4, W//4]
        x = self.pool(self.conv2(x))  # [B, 64, group, group]
        x = rearrange(x, 'b c h w-> b (h w) c') # [B, group*group, 64]
        x = self.mlp(x)  # [B, group*group, 64]
        x = self.end(x)  # [B, group*group, 256]
        return x
