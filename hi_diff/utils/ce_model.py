...
import numpy as np
from hi_diff.utils._basic_binary_modules import STEBinary_fc, ExpBinary_fc
...
class CEBlurNet(nn.Module):
    ...
    def __init__(self, sigma_range=0, test_sigma_range=0, ce_code_n=8, frame_n=8, ce_code_init=None, opt_cecode=False, binary_fc=None):
        ...
        if ce_code_init is None:
            ...
            ce_code_init = [-1 if ce_code_init[k] == 0 else 1 for k in range(len(ce_code_init))]
        ...
        for k in range(ce_code_n):
            self.upsample_matrix[k * self.upsample_factor:(k+1)*self.upsample_factor, k] = 1

    def forward(self, frames):
        device = frames.device
        ce_code = self.binary_fc(self.ce_weight)  # weights binarized
        ce_code_up = torch.matmul(
            self.upsample_matrix.to(device), ce_code.to(device))
