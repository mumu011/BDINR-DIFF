...
from hi_diff.utils.metric_iqa import calc_psnr, calc_ssim, compare_mse


@MODEL_REGISTRY.register()
class MU_Diff_S1(BaseModel):
    """MU-Diff model for test."""

    def __init__(self, opt):
        super(MU_Diff_S1, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if self.cri_pix is None and self.cri_reblur is None:
            raise ValueError('Both pixel and reblur losses are None.')
    ...
    def optimize_parameters(self, current_iter):
        ...
        # prior = self.net_le(self.lq, self.gt)  # img (8, 3, 128, 128) prior (8, 16, 256)
        # self.output = self.net_g(self.lq, prior) # output (8, 3, 128, 128)
        prior_list = self.net_le(ce_blur=self.lq, time_idx=self.time_idx, ce_code=self.ce_code_up, gt=self.gt) # [frame_n, b, 64, 128]
        output_list = []
        for k in range(self.frame_n):
            output_k = self.net_g(self.lq, prior_list[k])
            output_list.append(output_k)
        # # no diffusion
        # prior_list = self.net_le(ce_blur=self.lq, time_idx=self.time_idx, ce_code=self.ce_code_up, gt=None)
        # output_list = []
        # for k in range(self.frame_n):
        #     output_k = self.net_g(prior_list[k], prior=None)
        #     output_list.append(output_k)
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                prior_list = self.net_le(ce_blur=img, time_idx=self.time_idx, ce_code=self.ce_code_up, gt=self.gt)
                output_list = []
                for k in range(self.frame_n):
                    output_k = self.net_g(img, prior_list[k])
                    output_list.append(output_k)
                # # no diffusion
                # prior_list = self.net_le(ce_blur=img, time_idx=self.time_idx, ce_code=self.ce_code_up, gt=None)
                # output_list = []
                # for k in range(self.frame_n):
                #     output_k = self.net_g(prior_list[k], prior=None)
                #     output_list.append(output_k)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if with_metrics:
                ...
                for name, opt_ in self.opt['val']['metrics'].items():
                    ...
                    # output_all = torch.flatten(self.output, end_dim=1)
                    # target_all = torch.flatten(self.gt, end_dim=1)
                    # batch_size = self.ce_blur_img.shape[0]
                    # self.metric_results[name] += calc_psnr(output_all, target_all) * batch_size
