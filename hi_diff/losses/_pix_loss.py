...
@LOSS_REGISTRY.register()
@add2loss
class MyCharbonnierLoss(nn.Module):
    """Charbonnier Loss"""

    def __init__(self, weight=1.0, eps=1e-3):
        super(MyCharbonnierLoss, self).__init__()
        self.weight = weight
        self.eps = eps
...
class EdgeLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        loss = self.loss(self.laplacian_kernel(output),
                         self.laplacian_kernel(target))
        return loss
...
class FFTLoss(nn.Module):
    ...
    def forward(self, output, target):
        diff = torch.fft.fft2(output) - \
            torch.fft.fft2(target)
