...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            num_total = len(train_set) * dataset_enlarge_ratio
            validation_split = 0.01
            num_valid = int(num_total * validation_split)
            num_train = num_total - num_valid
            train_set, val_set = torch.utils.data.random_split(train_set, [num_train, num_valid])
...
def load_resume_state(opt):
    ...
    if resume_state_path is None:
        ...
        if opt['name'] == "GoPro_S2_MU":
            # pretrain_network_le_path = "/hdd/0/wj/HI-Diff/experiments/GoPro_S1_MU_200k_3/models/net_le_180000.pth"
            pretrain_network_le_path = "/hdd/0/wj/HI-Diff/experiments/GoPro_S1_MU_300k/models/net_le_296000.pth"
            opt['path']['pretrain_network_le'] = pretrain_network_le_path
            print(f"Set pretrain_network_le to {pretrain_network_le_path}")
